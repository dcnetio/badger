...
packages=$(go list ./... | grep github.com/dcnetio/badger/)
...
go test -v -race github.com/dcnetio/badger/skl
...
go test -timeout=25m -v -race github.com/dcnetio/badger --vlog_mmap=true

echo
echo "==> Starting tests with value log not mmapped..."
go test -timeout=25m -v -race github.com/dcnetio/badger --vlog_mmap=false
