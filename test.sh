...
packages=$(go list ./... | grep github.com/dcnetio/badger2/)
...
go test -v -race github.com/dcnetio/badger2/skl
...
go test -timeout=25m -v -race github.com/dcnetio/badger2 --vlog_mmap=true

echo
echo "==> Starting tests with value log not mmapped..."
go test -timeout=25m -v -race github.com/dcnetio/badger2 --vlog_mmap=false
